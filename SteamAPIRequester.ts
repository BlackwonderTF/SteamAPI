...
export async function getPlayerSummaries(...targets: string[]) {
  if (targets.length > 100) {
    console.error('Only 100 steamIDs can be requested at a time!')
    return undefined
  }

  try {
    let requestURL = `http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=${SteamAPIKey}&steamids=${targets}`
    ...
    if (returnArray.length >= 1) return returnArray
...
export async function resolveVanityUrl(vanity: string) {
  const requestURL = `http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${SteamAPIKey}&vanityurl=${vanity}`
